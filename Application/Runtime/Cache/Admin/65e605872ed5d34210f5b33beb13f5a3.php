                ...
                <form method="get" action="<?php echo U('index');?>">
                    <div style="float:left;height:55px;">
                        ...
                        <div class="head-search" style="margin-right: 0px;">
                            ...
                            <div class="form-input-btn select_1">                                <input style="width:230px;" type="text" name="clvar" class="form-input-small" placeholder="请输入昵称/姓名/手机号" id="clvar" value="<?php echo ($clvar); ?>">
                            </div>
                            ...
                            <div class="form-input-btn select_3">
                                <select name="clip_type"style="color: #666;width:159px;font-size: 14px;border:1px solid #eef1f1;border-radius:3px;height: 36px;margin-right: 5px;">
                                    <option value='0'<?php if($clip_type == 0): ?>selected="selected"<?php endif; ?>>购卡类型</option>
                                    <option value='1'<?php if($clip_type == 1): ?>selected="selected"<?php endif; ?>>卡片一</option>
                                    <option value='2'<?php if($clip_type == 2): ?>selected="selected"<?php endif; ?>>卡片二</option>
                                    <option value='3'<?php if($clip_type == 3): ?>selected="selected"<?php endif; ?>>卡片一二</option>
                                </select>
                            </div>
        ...
        $('.select_3').click(function(){
            $('.clsea').val(3);
                ...
                /*$('#clsea').change(function(){
                  if($(this).val()=='1'){
                    $('#clvar').attr('placeholder','请输入昵称/姓名/手机号');
                    $('.select_1').show();
                    $('.select_2').hide();
                  }
                  if($(this).val()=='2'){
                    $('#clvar').attr('placeholder','请输入级别');
                    $('.select_2').show();
                    $('.select_1').hide();
                  }
                  if($(this).val()=='3'){
        ...
        <div class="table-list">
            <table cellpadding="0" cellspacing="0">
                ...
                <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
                      ...
                      <td> <?php echo ((isset($v["iphone"]) && ($v["iphone"] !== ""))?($v["iphone"]):"-"); ?> </td>
                      <td> <?php echo ((isset($v["address"]) && ($v["address"] !== ""))?($v["address"]):"-"); ?> </td>
                      <td> 
                        <?php if($v['clip_type'] == 1): ?>会员卡一<?php endif; ?> 
                        <?php if($v['clip_type'] == 2): ?>会员卡二<?php endif; ?> 
                        <?php if($v['clip_type'] == 3): ?>会员卡一二<?php endif; ?> 
                        <?php if($v['clip_type'] == 0): ?>无<?php endif; ?> 
                        </td>
                      <td> <?php echo ($v["lev_name"]); ?> </td>
                      <td> <?php echo ($v["date"]); ?> </td>
                      <td class="surplus-money"> <?php echo ($v["surplus_money"]); ?> </td>
                      <td> <?php echo ($v["surplus_int"]); ?> </td>
                      <td>
