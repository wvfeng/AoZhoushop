...
<div class="pop-up-bj pop-up-delete none">
    ...
    <div class="info-main main FromUpToDown">
        <h1>商品编辑 <a href="javascript:history.go(-1);"style="float:right;">返回上一级</a></h1>
        <form method="post" action="<?php echo U('Shop/doadd');?>" id="postform">
            <div class="info-list clearfix">
                <div class="info-right" style="width: 700px;">
                    <div class="info-model clearfix">
                        ...
                        <div>
                            <select name="classify_id" class="is_vip" style="color: #666;width:199px;font-size: 14px;border:1px solid #eef1f1;border-radius:3px;height: 36px;margin-right: 5px;">
                                    ...
                                    <?php if(is_array($classify)): $i = 0; $__LIST__ = $classify;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><option value="<?php echo ($v["id"]); ?>"<?php if($res['classify_id'] == $v['id']): ?>selected="selected"<?php endif; ?>><?php echo ($v["classname"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                            </select>
                        </div>
                    </div>
                    <div class="info-model clearfix"id="_type"style="width:800px;">
                        ...
                        <?php if(is_array($res["type"])): $i = 0; $__LIST__ = $res["type"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><input type='hidden' name='type_all[]' value='<?php echo ($v); ?>'><span><a title='点击删除' style='color:red;cursor: pointer;'><?php echo ($v); ?></a></span><?php endforeach; endif; else: echo "" ;endif; ?>
                    </div>
                    ...
                    <div class="info-model clearfix">
                        ...
                        <div><input type="text" name="tit" value="<?php echo ($res["tit"]); ?>" placeholder="名称"/></div>
                    </div>
                    <div class="info-model clearfix">
                        <span>商品名称(en)</span>
                        <div><input type="text" name="tit_en" value="<?php echo ($res["tit_en"]); ?>" placeholder="名称(en)"/></div>
                    </div>
                    <div class="info-model clearfix">
                    ...
                    <div class="info-model clearfix">
                        ...
                        <div style="height:80px;" class="slp">
                            <?php if(is_array($res["sliedimg"])): $i = 0; $__LIST__ = $res["sliedimg"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><img style='width:80px;height:80px;'src='/azshop/Public/shopimg/<?php echo ($v); ?>'>
                                <input type='hidden' name='slide[]' value='<?php echo ($v); ?>'>
                                <b class="sremove">删除</b><?php endforeach; endif; else: echo "" ;endif; ?>
                            <img src="/azshop/Public/Admin/images/up.jpg"style="width:80px;height:80px;" id="browse_slid">
                        </div>
                    </div>
                    <div class="info-model clearfix">
                        <span>所需现金</span>
                        <div><input type="number" name="price" value="<?php echo ($res["price"]); ?>" placeholder="现金"/></div>
                    </div>
                    <div class="info-model clearfix">
                        <span>商品规格</span>
                        <div><input type="text" name="specifications" value="<?php echo ($res["specifications"]); ?>" placeholder="规格"/></div>
                    </div>
                    <div class="info-model clearfix">
                        <span>商品产地</span>
                        <div><input type="text" name="origin" value="<?php echo ($res["origin"]); ?>" placeholder="产地"/></div>
                    </div>
                    <div class="info-model clearfix">
                        <span>储存方法</span>
                        <div><input type="text" name="storage" value="<?php echo ($res["storage"]); ?>" placeholder="储存方法"/></div>
                    </div>
                    <div class="info-model clearfix">
                        <span>商品汇率</span>
                        <div><input type="text" name="rate" value="<?php echo ($res["rate"]); ?>" placeholder="汇率"/></div>
                    </div>
                    <div class="info-model clearfix">
                        <span>商品详情</span>
                        <div style="height:auto;">
                            <textarea name="detail" id="container" style="height:300px;width:800px" ><?php echo ($res["detail"]); ?></textarea>
                        </div>
                    </div>
                    <div class="info-model clearfix">
                        <span>商品详情(en)</span>
                        <div style="height:auto;">
                            <textarea name="detail_en" id="container_en" style="height:300px;width:800px" ><?php echo ($res["detail_en"]); ?></textarea>
                        </div>
                    </div>
                </div>
            </div>
    ...
    uploader_slid.bind('FileUploaded',function(uploader_slid,files,responseObject){
      ...
      $('#browse_slid').before("<b class='sremove'>删除</b>");
    });
